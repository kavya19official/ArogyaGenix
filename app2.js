...
function initVitalsChart() {
  ...
  // Synthetic data generation for the week
  const bpSysData = [120, 125, 130, 128, 135, 142, 140];
  const bpDiaData = [80, 82, 85, 84, 88, 92, 90];
  const glucoseData = [100, 105, 110, 108, 115, 125, 120];
...
function initLogin() {
  ...
  const patientBtn = document.getElementById('btn-login-patient');
  const doctorBtn = document.getElementById('btn-login-doctor');
  ...
  function showDashboard(role) {
    ...
    // Record login event
    recordAction(`${role.charAt(0).toUpperCase() + role.slice(1)} logged in`);
  }

  patientBtn.addEventListener('click', () => showDashboard('patient'));
  doctorBtn.addEventListener('click', () => showDashboard('doctor'));
}

// Update DOMContentLoaded to include login init
document.addEventListener('DOMContentLoaded', () => {
  initLogin();
});
